<?php
    ...
    if(!isN($_pm_module)){
		...
		if($_pm_section == 'process' || $_pm_section == 'data'){
            ...
            if( ChckSESS_cnt() ){
                if( !isN($_SESSION['org_mrks']) && !isN($_GET['i']) && ( $_SESSION['org_mrks'] != $_GET['i'] ) ){
                    header('Location:'.$_sbd.'/'.$_pm_module.'/dashboard/?i='.$_SESSION['org_mrks']);
                }else if( !isN($_SESSION['org_mrks']) && isN($_GET['i']) ){
                    header('Location:'.$_sbd.'/'.$_pm_module.'/dashboard/?i='.$_SESSION['org_mrks']);
                }
            }
