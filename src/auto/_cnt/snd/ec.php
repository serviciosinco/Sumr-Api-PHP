...
if( $_g_alw->est == 'ok' ){
	...
	if(class_exists('CRM_Cnx')){
		...
		if($this->g__s3 == 'cmpg_snd'){
			echo $this->h1('(OLD) ENVIO DE CAMPANA');
		}else{
			echo $this->h1('(OLD) ENVIO DE PUSHMAIL');
		}
			...
			foreach($this->_s_cl->ls as $_cl_k=>$_cl_v){
				...
				if( $this->tallw_cl([ 't'=>'key', 'id'=>'snd_ec', 'cl'=>$_cl_v->id ])->est == 'ok' ){
					...
					//-------------------- AUTO TIME CHECK - START --------------------//
						...
						$_AUTOP_d = $this->RquDt([ 't'=>$__rqu_tp, 'cl'=>$_cl_v->id, 's'=>10 ]);
						$_AUTOP_d->e = 'ok';
						$_AUTOP_d->lck = 'no';
						...
						if($__cl_rqu == 'ok'){ //----------- CHECK CLIENT ALLOWED -----------//
							...
							//---------- Filter Campaign ----------//
								...
								if($this->g__s3 == 'cmpg_snd'){
									...
									require(GL_SND_EC.'c_filter.php');
								}else{
									$__snd_vw = VW_EC_SND;
								}
							...
							if($___lck->e == 'ok' && !isN($EcSndQry) && !isN($__snd_vw)){
								...
								$EcSnd = $__cnx->_prc($EcSndQry,['cmps'=>'ok']); //Read main database, no problems with lag AWS
								...
								if($TotEcSnd > 0){
									...
									//---------- Lock All Records On This Query Result ----------//
										...
										if($__snd_lock_qry->e == 'ok' && $___unlck->e == 'ok'){
											...
											//TODO: Quit this after migrate to AWS Bulk Mail, Loop Controlled by Extension File

											foreach($___datprcs as $___datprcs_k=>$___datprcs_v){
												require(GL_SND_EC.'tempo_batch.php');
											}

											//FIXME: Test and deploy aws bulk mail 50 in one process
											/*if(!isN($__cmpg_to_work_id)){
