	...
	try {
		...
		if(in_array($___datprcs_v['id_ecsnd'], $___otprcs)){
			...
			if($___allw_snd == 'ok'){
				...
				if( $___chk->e == 'ok' &&
					(	$___chk->btch === $this->__btch_id ||
						(!isN($___chk->diff) && $___chk->diff->i > 10) ||
						$___datprcs_v['__rd_aft']
					)
				){
					...
					try {
						...
						if(!isN($__us_msj) && !isN($__us_eml) && !isN($__us_as)){
							...
							if(!isN($__cmpg_dt->id)){
								if(trim($__us_as) == trim($__cmpg_dt->sbj)){
									require('tempo_send.php');
								}else{
							...
							if(!isN($__cmpg_dt->id)){
								$_upd_eml_rdy = $_ec_snd->_EcCmpgUpd_Fld([ 'id'=>$__cmpg_dt->id, 'f'=>'eccmpg_rdy', 'v'=>2 ]);
							}

							echo $this->err('No code HTML on old send');
