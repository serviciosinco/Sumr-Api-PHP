		...
		if($this->g__s3 == 'cmpg_snd'){
			...
			if($this->g__i){
				...
				if(!isN($__cmpg_to_work->id)){
					...
					echo $this->err('No campaign on '.$__cmpg_to_work->qry);
				}
			...
			if(!isN($__cmpg_to_work_id) && !isN($__cmpg_to_work_id_f)){
				...
				//$__cmpg_slc = ', id_eccmpg AS __idcmpg';
				
				$__cmpg_innr = ' 	
					INNER JOIN '._BdStr($_cl_v->bd).TB_EC_SND_CMPG.' ON ecsndcmpg_snd = id_ecsnd
				...
				try {	
		   		...
		   		//echo $this->err('No next campaign so lets continue random '.$__cmpg_to_work->qry);  
			...
			$__cmpg_innr = 'LEFT JOIN '._BdStr($_cl_v->bd).TB_EC_SND_CMPG.' ON ecsndcmpg_snd = id_ecsnd';
