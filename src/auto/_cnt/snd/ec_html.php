						...
						try {
							...
							//---------- Filter Campaign ----------//
								...
								if($this->g__s3 == 'cmpg'){
									...
									include(GL_SND_HTML.'cmpg.php');
									if(isN($__cmpg_to_work_id)){ continue; }
								}else{
									$__snd_vw = VW_EC_SND;
									$__ord_by = 'ecsnd_f, ecsnd_h ASC';
								}
									...
									if($__snd_lock_qry->e == 'ok' && $___unlck->e == 'ok'){
										...
										if($this->g__s3 == 'cmpg'){
											...
											if(!isN($__cmpg_to_work->id)){
												$_upd_cmpg_rdy = $__ec->_EcCmpgUpd_Fld([ 'id'=>$__cmpg_to_work->id, 'f'=>'eccmpg_tot_upd', 'v'=>1 ]);
												echo $this->li( 'Ready campaign tot update: '.$_upd_cmpg_rdy->e );
											}elseif(!isN($__cmpg_dt->id)){
												$_upd_cmpg_rdy = $__ec->_EcCmpgUpd_Fld([ 'id'=>$__cmpg_dt->id, 'f'=>'eccmpg_tot_upd', 'v'=>1 ]);
												echo $this->li( 'Ready campaign tot update: '.$_upd_cmpg_rdy->e );
											}else{
												$_upd_cmpg_rdy=NULL;
											}

											if($_upd_cmpg_rdy->e == 'ok'){
