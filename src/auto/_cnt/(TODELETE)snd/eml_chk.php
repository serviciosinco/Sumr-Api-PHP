			...
			foreach($this->_s_cl->ls as $_cl_k=>$_cl_v){
				...
				if( $this->tallw_cl([ 't'=>'key', 'id'=>'snd_eml_chk', 'cl'=>$_cl_v->id ])->est == 'ok' ){
					...
					if($_AUTOP_d->e == 'ok' && $_AUTOP_d->hb == 'ok'){
						...
						if(!isN($this->g__lmt)){ $_qry_lmt = $this->g__lmt; }else{ $_qry_lmt = '20'; }
						...
						if(!isN($___datprcs)){
							...
							foreach($___datprcs as $___datprcs_k=>$___datprcs_v){
								...
								$_dmn_eml = array_pop(explode('@', $___datprcs_v['cnteml_eml']));
								$_chk_eml = Chk_EmlRle([  'eml'=>$___datprcs_v['cnteml_eml'] ]);

								echo $this->h1('Start Check For:'.$___datprcs_v['cnteml_eml']);
								echo $this->h2('Chk_EmlRle:'.$_chk_eml->e);

								if(!checkdnsrr($_dmn_eml, 'MX')){

									$__prc = UPDCntEml([ 'bd'=>$_cl_v->bd, 'id'=>$___datprcs_v['cnteml_enc'], 'est'=>_CId('ID_SISEMLEST_NOEXST') ]);

									if($__prc->e == 'ok'){
										echo $this->h2("Mal: ".$___datprcs_v['cnteml_eml']." no existe - estado "._CId('ID_SISEMLEST_NOEXST'));
									}

								}elseif($_chk_eml->e == 'no' && filter_var( $___datprcs_v['cnteml_eml'], FILTER_VALIDATE_EMAIL )){

									$__prc = UPDCntEml([ 'bd'=>$_cl_v->bd, 'id'=>$___datprcs_v['cnteml_enc'], 'est'=>_CId('ID_SISEMLEST_ACT') ]);

									if($__prc->e == 'ok'){
										echo $this->h2("Bien: ".$___datprcs_v['cnteml_eml']." formato correcto - estado "._CId('ID_SISEMLEST_ACT'));
										//echo $this->scss( print_r($__prc, true) );
									}

								}elseif($_chk_eml->e == 'ok'){

									$__prc =  UPDCntEml([ 'bd'=>$_cl_v->bd, 'id'=>$___datprcs_v['cnteml_enc'], 'est'=>_CId('ID_SISEMLEST_BADFRMT') ]);

									if($__prc->e == 'ok'){
										echo $this->h2("Mal: ".$___datprcs_v['cnteml_eml']." formato erroneo - estado "._CId('ID_SISEMLEST_BADFRMT'));
									}

								}else{

									echo $this->err($___datprcs_v['cnteml_eml']." No hay respuesta: ".$_chk_eml->e);
