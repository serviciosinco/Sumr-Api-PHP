<?php 
	...
	$decode = GtDtIgr($_fn, $_min, $_mx); print_r($decode);
	$_n = 0;
?> 

<?php if(is_array($decode->data)){ ?>
<ul class="msghsh"> 
					<?php foreach(array_slice($decode->data, 0, $limit) as $result){  ?>
									  <?php if(ChckIdQus(array('id'=>$result->id, 'tp'=>'2'))){ $_clsitm = 'item_qus'; }elseif(ChckIdMsj($result->id)){ $_clsitm = 'item_ok'; }else{ $_clsitm = 'item'; }?>
									  
									  <?php $__i = $result->id; if($_n == 0){ $__n = $__i;} $_n++;?>
									  
												  <li id="Li_<?php echo $result->id ?>">
                                                  
                                                  		<?php $__imgprf_big = $result->user->profile_picture; ?>
                                                  		
														<a href="<?php echo $__imgprf_big ?>" class="__clrbx"><img src="<?php echo $result->user->profile_picture ?>" class="_prf _prf_igr"></a>
														<div class="<?php echo $_clsitm ?>" id="Dv_<?php echo $result->id ?>">
															<div class="ldr"></div>
															
															<?php $__media = $result->images->standard_resolution->url; $__media_th = $result->images->thumbnail->url; if($__media != NULL && $__media!=''){ ?>
                                                            
                                                                <a href="<?php echo $__media ?>" class="__clrbx">
                                                                		<img src="<?php echo $__media_th ?>" class="_r">
                                                                </a>
                                                                
                                                            <?php } ?>
                                                            
                                                            <h2><?php echo $result->user->full_name.' <span>@'.$result->user->username.'</span>'/*.' ('.$__i.')'*/ ?></h2>
															<?php echo $result->caption->text ?>
                                                            <div class="_date"><?php echo Spn(date('Y-m-d', $result->created_time),'','_f').' / '.Spn(date('G:i:s', $result->created_time),'','_h') . Spn($result->id, 'ok') ?></div>
															...
															<div class="btn">  
																	<div class="aprb">	
																		  <form name="FmMsgTw_<?php echo $result->id ?>" id="FmMsgTw_<?php echo $result->id ?>" method="post" action="<?php echo PRC_HSH ?>">
																				...
																				<input name='hshmsg_idstr' type='hidden' id='hshmsg_idstr' value='<?php echo $result->id ?>' />
																				...
																				<input name='hshmsg_profileimageurl' type='hidden' id='hshmsg_profileimageurl' value='<?php echo $result->user->profile_picture ?>' />
																				<input name='hshmsg_source' type='hidden' id='hshmsg_source' value='<?php echo $result->source ?>' />
																				<input name='hshmsg_fromusername' type='hidden' id='hshmsg_fromusername' value='<?php echo $result->user->full_name ?>' />
																				<input name='hshmsg_fromuser' type='hidden' id='hshmsg_fromuser' value='<?php echo $result->user->username ?>' />
																				<input name='hshmsg_text' type='hidden' id='hshmsg_text' value='<?php echo $result->caption->text ?>' />
																				<input name='hshmsg_hsh' type='hidden' id='hshmsg_hsh' value='<?php echo $_hshtg_svid ?>' />
                                                                                <input name='hshmsg_media' type='hidden' id='hshmsg_media' value='<?php echo $result->images->standard_resolution->url ?>' />
                                                                                <input name='hshmsg_tp' type='hidden' id='hshmsg_tp' value='2' />
	...
	<script type="text/javascript">
		$(document).ready(function(){
			<?php echo $_phpjq; ?>
			<?php if($__n != ''){ ?>$('#Igr_Frs').val("<?php echo $decode->pagination->next_min_id ?>");<?php } ?>
			<?php if($__i != ''){ ?>$('#Igr_Lst').val("<?php echo $decode->pagination->next_max_id ?>");<?php } ?>
