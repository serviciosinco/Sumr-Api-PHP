...
class Instagram {
   ...
   * The OAuth token URL
   */
  const APITW_OAUTH_TOKEN_URL = 'https://api.instagram.com/oauth/access_token';
   ...
   * @return mixed
   */
  public function getTagMedia($name, $count, $max, $min) {
    return $this->_makeCall('tags/'.$name.'/media/recent',false, array('count'=>$count, 'max_tag_id'=>$max, 'min_tag_id'=>$min));
  }
  ...
  private function _makeOAuthCall($apiData) {
    $apiHost = self::APITW_OAUTH_TOKEN_URL;
