...
if(class_exists('CRM_Cnx')){
	...
	if(_Chk_VLE('_cntAre', 'p')){ $__fl .= ' AND mdlcnt_mdl IN ( SELECT mdlare_mdl  FROM '.$__dt_cl->bd.'.'.TB_MDL_ARE.'  INNER JOIN '._BdStr(DBM).TB_CL_ARE.' ON mdlare_are = id_clare
																	WHERE id_clare IN ( '.$_POST['_cntAre'].' )
																)  '; }
	...
	$__dwn = _Dwn_S([
						't'=>'mdl_cnt',
						'us'=>_GetPost('_us'),
						'e_s'=>_GetPost('_eml_snd'),
						'frm'=>_GetPost('_frm'),
						'vl_p'=>_GetPost('prgm_chk'),
						'vl_t_p'=>_GetPost('dwn_tm_prg_vl'),
						'vl_t_d_1'=>_GetPost('dwn_tm_prg_d_1'),
						'vl_t_d_2'=>_GetPost('dwn_tm_prg_d_2'),
						'vl_t_d_3'=>_GetPost('dwn_tm_prg_d_3'),
						'vl_t_d_4'=>_GetPost('dwn_tm_prg_d_4'),
						'vl_t_d_5'=>_GetPost('dwn_tm_prg_d_5'),
						'vl_t_d_6'=>_GetPost('dwn_tm_prg_d_6'),
						'vl_t_d_7'=>_GetPost('dwn_tm_prg_d_7'),
						'vl_t_bfr'=>_GetPost('dwn_tm_prg_bfe')
					]);
	...
	if(!isN($__dwn->id)){
		...
		if($__dwn_col->ls->intrs->e == 'ok'){
			$_dwn_flds_col .= ", (SELECT COUNT(*) FROM ".$__dt_cl->bd.".".TB_MDL_CNT." WHERE mdlcnt_cnt = id_cnt) AS Intereses";
		}
		...
		$Ls_Qry = "
			...
			CREATE TABLE IF NOT EXISTS "._BdStr(DBD).$__dwn->tab." ENGINE=InnoDB ROW_FORMAT=COMPACT AS (
				...
				SELECT
					...
					".DBD.".ctjTx( IF(cplcy.cntplcy_sndi=1, cnt_nm, '-anonimo-') ) AS Nombres,
					".DBD.".ctjTx( IF(cplcy.cntplcy_sndi=1, cnt_ap, '') ) AS Apellidos,
		...
		$Ls_Qry_His = "
			...
			CREATE TABLE IF NOT EXISTS "._BdStr(DBD).$__dwn->tab."_his ENGINE=MyISAM CHARSET=utf8 COLLATE utf8_general_ci ROW_FORMAT=COMPACT AS (
				SELECT
					id_mdlcnt AS ID,
					id_mdlcnt AS __dwn_i
				   	$Ls_Whr $__fl $Ls_WhrO
		...
		$__sve_qry = _Dwn_His_S([ "dwn"=>$__dwn->id, "main"=>$Ls_Qry, 'his'=>$Ls_Qry_His ]);
		//$rsp['qry'] = compress_code($Ls_Qry);
		//$rsp['qry_s'] = $__sve_qry;
		...
		if(!isN($__dwn->id) && $__sve_qry->e == 'ok'){
			...
			$rsp['w'] = 'Query not saved on '.$__sve_qry->w;
