...
if(class_exists('CRM_Cnx')){
	...
	if(!isN($___Ls->gt->i)){
		...
		$___Ls->qrys = sprintf("SELECT * FROM "._BdStr(DBD).TB_DWN_COL." WHERE ".$___Ls->ik." = %s LIMIT 1", GtSQLVlStr($___Ls->gt->i, "text"));
		
	}elseif($___Ls->_show_ls == 'ok'){ 
		  
		$Ls_Whr = "	FROM "._BdStr(DBD).TB_DWN_COL." 
						 ".GtSlc_QryExtra([ 't'=>'tb', 'col'=>'dwncol_col', 'als'=>'c' ])."
					WHERE 	".$___Ls->ino." != '' AND 
							dwncol_cl = ".DB_CL_ID." ".$___Ls->sch->cod." 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="Ls_Rg">
  	...
  	<tr>
		...
		<td width="1%" align="left" <?php echo $_clr_rw ?>><?php echo _sino($___Ls->ls->rw['dwncol_e']); ?></td>
    	...
    	<form method="POST" name="<?php echo $___Ls->fm->id; ?>" target="_self" id="<?php echo $___Ls->fm->id; ?>" >
	      	...
	      	<div id="<?php echo $___Ls->fm->fld->id ?>" class="<?php echo DV_GNR_FM_CMP ?>">
		        <div class="ln_1">
				  	...
				  	<div class="col_2">	   
						<?php 
							echo OLD_HTML_chck('dwncol_e', TX_EST, $___Ls->dt->rw['dwncol_e'] );
						?>
		          	</div>
		        </div>
      		</div>
