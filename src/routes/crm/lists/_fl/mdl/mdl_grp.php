...
if(class_exists('CRM_Cnx')){
	...
	if(!isN($___Ls->gt->i)){
		...
		$___Ls->qrys = sprintf("SELECT * FROM ".TB_MDL_GRP." WHERE ".$___Ls->ik." = %s LIMIT 1", GtSQLVlStr($___Ls->gt->i, "text")); 
        ...
        if(_SbLs_ID('i') && $___Ls->gt->tsb == 'mdl'){	
			...
			$__fl .= " AND clgrp_enc = ".GtSQLVlStr($___Ls->gt->isb, "text")." "; 	
		}	

		$Ls_Whr = "	FROM ".TB_MDL_GRP."
                         INNER JOIN ".TB_MDL." ON mdlgrp_mdl = id_mdl
        ...
        $___Ls->qrys = "SELECT  mdl_nm, clgrp_nm, mdlgrp_clgrp, mdlgrp_enc,
				...
				<tr>    
					...
					<?php if($___Ls->gt->tsb == 'mdl'){ ?>
						<td width="49%" align="left" nowrap="nowrap">
							<?php echo ShortTx(ctjTx($___Ls->ls->rw['clgrp_nm'],'in'),40,'Pt', true);
							?>
						</td>
					<?php }else{ ?>
						...
						<div class="ln_1">
							...
							<?php 
								...
								if($___Ls->gt->tsb == 'mdl'){
									...
									echo HTML_inp_hd('mdlgrp_mdl', $__mdl_d->id);
									echo LsClGrp('mdlgrp_clgrp','id_clgrp', $___Ls->dt->rw['mdlgrp_clgrp'], 'Grupo', 2, '');  $CntWb .= JQ_Ls('mdlgrp_clgrp','Grupo'); 
								}else{
									$__cl_are = GtClGrpDt([ 'enc'=>$___Ls->gt->isb ]);
									echo HTML_inp_hd('mdlgrp_clgrp', $__cl_are->id);
									echo LsMdl('mdlgrp_mdl', 'id_mdl', $___Ls->dt->rw['mdlgrp_mdl'], TX_MDL, 2, '', [ 'prfx'=>'mdlstp_nm' ]); $CntWb .= JQ_Ls('mdlgrp_mdl', TX_MDL);
								}

							?> 

                            <?php 
                                $l = __Ls([ 'k'=>'us_rol', 'id'=>'mdlgrp_tp', 'v'=>'', 'va'=>$___Ls->dt->rw['mdlgrp_tp'], 'ph' =>'' ]);
