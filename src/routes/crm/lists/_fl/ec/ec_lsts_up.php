...
if(class_exists('CRM_Cnx')){
	...
	$___Ls->cnx->cl = 'ok';
	$___Ls->tpr = 'ec_lsts';
	$___Ls->sch->f = 'eclstsup_nm';
	$___Ls->ino = 'id_eclstsup';
	$___Ls->ik = 'eclstsup_enc';
	$___Ls->new->w = 400;
	$___Ls->new->h = 300;	 
	$___Ls->edit->w = 600;
	$___Ls->edit->h = 600;
	...
	if(_SbLs_ID('i')){
		...
		$__fl .= " AND eclstsup_lsts = ( 	SELECT id_eclsts
	...
	if(!isN($___Ls->gt->i)){	 			 
		...
		$Ls_TotLds = ", (SELECT COUNT(*) FROM ".DB_PRC.".".MDL_UP_COL_BD." WHERE upcol_up = eclstsup_up) AS __tot_lds ";
		$Ls_TotLds_Ld = ", (SELECT COUNT(*) FROM ".TB_EC_LSTS_EML." WHERE eclstseml_up = id_eclstsup) AS __tot_lds_ld ";
		$Ls_TotLds_W = ", (SELECT COUNT(*) FROM ".DB_PRC.".".MDL_UP_COL_BD." WHERE upcol_up = eclstsup_up AND upcol_est = 3) AS __tot_lds_w ";
		$Ls_TotLds_P = ", (SELECT COUNT(*) FROM ".DB_PRC.".".MDL_UP_COL_BD." WHERE upcol_up = eclstsup_up AND upcol_est = 2) AS __tot_lds_p ";
	
		$Ls_Whr = "	FROM "._BdStr(DBM).TB_EC_LSTS_UP." 
						 INNER JOIN "._BdStr(DB_PRC).TB_UP_BD." ON eclstsup_up = id_up
						 INNER JOIN "._BdStr(DBM).TB_EC_LSTS." ON eclstsup_lsts = id_eclsts
						 INNER JOIN "._BdStr(DBM).TB_CL." ON eclsts_cl = id_cl
						 ".GtSlc_QryExtra([ 't'=>'tb', 'col'=>'up_est', 'als'=>'e' ])."
					WHERE id_eclstsup != '' $_f_tp $__fl AND cl_enc = '".DB_CL_ENC."' 
					ORDER BY id_eclstsup DESC";
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="Ls_Rg LsRgNw">
  <tbody>
		  ...
		  <tr <?php 
	            ...
	            $__DtUp = GtUpColTot(['id'=>$___Ls->ls->rw['eclstsup_up']]);       
          ...
          ?>>
            <td align="left" <?php echo $_clr_rw ?> width="84%" class="__up_ls">
	            	<?php echo h2( 'Carga '.$___Ls->ls->rw['id_up'] /*ctjTx($___Ls->ls->rw['up_fle'],'in')*/ ).
		            		   ...
		            		   Strn(TX_FICR).' '.
		            		   Spn(_DteHTML(['d'=>$___Ls->ls->rw['eclstsup_fi'], 'nd'=>'ok']), '', '_f');
		           	?>
		    </td>
              	...
              	$CntJV .= "
              		...
              		function f_up_updt_domrbld(){
              			...
              			$('.___upshowdtl').off('click').click(function(e){
							...
							_ldCnt({ 
								u:'".FL_UP_GN.__t('snd_ec_lsts_up', true).ADM_LNK_DT."'+__id+'"._SbLs_ID()."&_w=ok', 
								pop:'ok',
								pnl:{
									e:'ok',
									s:'l',
