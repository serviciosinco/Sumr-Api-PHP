					...
					$CntJV .=	"
							...
							function __mnucl_rbldom(){
					       		...
					       		$('.__cl_item ._opt button').not('._add').off('click').on('click', function(e){ 
									...
									$('#clmnur_clr_fnt').val( __set_clr_fnt );
									$('#clmnur_clr_bck').val( __set_clr_bck );
								...
								$('.__cl_slc .more_opt ._ovr').off('click').on('click', function(e){	
									...
									__g_clr_fnt = $('#clmnur_clr_fnt').val(); 
									__g_clr_bck = $('#clmnur_clr_bck').val();
											
									if( __set_clr_fnt != __g_clr_fnt || __set_clr_bck != __g_clr_bck ){ 
										
										__set_clr_fnt = $('#clmnur_clr_fnt').val(); 
										__set_clr_bck = $('#clmnur_clr_bck').val();
						   	...
						   	function __mnucl_rqu(p=null){

								if (SUMR_Main.onl() && isN( SUMR_Main.ibx['mnu_cl'] ) ){
									
									SUMR_Main.ibx['mnu_cl'] = $.ajax({
															type:'POST',
															dataType: 'json',
							...
							function __AddItm(__v){
								...
								if(!isN(__v.__mnu) && !isN(__v.__mnu.clr) && !isN( __v.__mnu.clr.bck) ){ 
									mnu_clr_bck = __v.__mnu.clr.bck 
								}else{
									mnu_clr_bck = '';
								}
								
								if(!isN(__v.__mnu) && !isN(__v.__mnu.clr) && !isN( __v.__mnu.clr.fnt) ){ 
									mnu_clr_fnt = __v.__mnu.clr.fnt 
								}else{
									mnu_clr_fnt = '';
								}
								
								
								if(!isN(__v.__mnu) && !isN(__v.__mnu.enc)){ 
									mnu_enc = __v.__mnu.enc
								}else{
									mnu_enc = '';
								}
								
								
								var __h = '<li id=\"'+__li_id+'\" class=\"__cl_item '+ __cls_sl +' '+ _b_c +'\" style=\"background-color:'+ __v.__clr +'\" rlc-id=\"'+ mnu_enc +'\">
													<div class=\"_opt\">
														...
														<button class=\"_clr _on\" clr-bck=\"'+ mnu_clr_bck +'\" clr-fnt=\"'+ mnu_clr_fnt +'\" enc-id=\"'+ mnu_enc +'\" ></button>
													</div>
							...
							function __snd_upd(__v){
			                   		...
			                   		if(__v.__t == 'a'){ 
			                   			...
			                   			var _MM = 'MMR_delete'; 
			                   			var _IlI = 'Act_'+__v.__t_prc+'_'+__v.__i_mnu; 
			                   			var _Uli = '{$__id_div_psb}';
			                   			var _t_go = 'a'; 
			                   		}
