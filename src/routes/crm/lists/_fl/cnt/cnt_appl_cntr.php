...
if(class_exists('CRM_Cnx')){
	...
	if(!isN($___Ls->gt->i)){
		...
		$Ls_Whr = "FROM ".TB_CNT_APPL_ATTR."
						".GtSlc_QryExtra(['t'=>'tb', 'col'=>'cntapplattr_attr', 'als'=>'t'])."
						LEFT JOIN "._BdStr(DBM).TB_SIS_SLC_F." ON id_sisslc = sisslcf_slc
						LEFT JOIN "._BdStr(DBM).TB_SIS_SLC_TP_F." ON sisslcf_f = id_sisslctpf
						WHERE id_cntapplattr != '' $_fl ".$___Ls->sch->cod."
						
						/*AND sisslc_tp = 134*/
						AND sisslc_tp IN (SELECT id_sisslctp FROM "._BdStr(DBM).TB_SIS_SLC_TP." WHERE sisslctp_key = 'appl_attr')
